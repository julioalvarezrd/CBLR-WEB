    ...
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-[74px] w-full max-w-[1480px] items-center gap-8 px-4 sm:px-6 lg:px-8">
          <Link href="/seguridad" aria-label="Ir al inicio de SIBOR">
            <SiborBrand compact />
          </Link>
            ...
            {navigation.map((group) =>
              group.items && group.items.length > 0 ? (
                  ...
                  <div className="absolute left-0 top-[calc(100%+10px)] min-w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
                    {group.items.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="block rounded-lg px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-slate-950"
                      >
                        {item.label}
                      </Link>
                    ))}
                ...
                <Link
                  key={group.label}
                  href={group.href}
                  className="rounded-lg px-2 py-2 text-sm font-medium text-slate-700 hover:text-slate-950"
                >
                  {group.label}
                </Link>
              ) : null,
            ...
            <details className="group relative">
              ...
              <div className="absolute right-0 top-[calc(100%+10px)] w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
                ...
                <form action={logoutAction}>
                  <button
                    type="submit"
                    className="w-full rounded-lg px-3 py-2.5 text-left text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-950"
                  >
                    Cerrar sesión
                  </button>
                </form>
          ...
          <button
            ...
            className="ml-auto grid size-10 place-items-center rounded-lg text-slate-700 hover:bg-slate-100 lg:hidden"
          >
            <MenuIcon />
          </button>
        </div>
                  ...
                  <div key={group.label}>
                    {group.href && (!group.items || group.items.length === 0) ? (
                      <Link
                        href={group.href}
                        onClick={() => setMobileOpen(false)}
                        className="block rounded-lg px-3 py-3 text-sm font-semibold text-slate-900 hover:bg-slate-50"
                      >
                        {group.label}
                      </Link>
                    ) : (
                        ...
                        <div className="mt-2 space-y-1">
                          {group.items?.map((item) => (
                            <Link
                              ...
                              href={item.href}
                              onClick={() => setMobileOpen(false)}
                              className="block rounded-lg px-3 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-950"
                            >
                              {item.label}
                            </Link>
                          ))}
