...
export default async function UsersPage({ searchParams }: UsersPageProps) {
  ...
  const [directory, counts] = await Promise.all([
    listUsers({ query, status, page, pageSize }),
    getUserCounts(),
  ]);
  ...
  return (
    <div className="space-y-6">
      <ModuleHeader
        ...
        stats={[
          { label: "Usuarios activos", value: counts.active, description: "Con acceso habilitado" },
          { label: "Inactivos", value: counts.inactive, description: "Acceso deshabilitado" },
          { label: "Con roles", value: counts.withRoles, description: "Con permisos asignados" },
          { label: "Sin roles", value: counts.withoutRoles, description: "Requieren asignación" },
        ]}
      />
      ...
      <ContentPanel
        ...
        description="Haz clic sobre un usuario para consultar su cuenta, roles y vínculo con Personal."
        trailing={
          ...
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="bg-slate-50/80 text-slate-600 dark:bg-slate-950/50 dark:text-slate-400">
              <tr>
                ...
                <th className="px-6 py-3.5 font-semibold">Último acceso</th>
                ...
                return (
                  <NavigableTableRow key={user.id} href={href}>
                    <td className="px-6 py-3.5">
                      <Link
                        href={href}
                        className="font-semibold text-slate-900 hover:text-red-700 dark:text-slate-100 dark:hover:text-red-400"
                      >
                        {displayName}
                      </Link>
                    ...
                    <td className="max-w-md px-6 py-3.5 text-slate-600 dark:text-slate-300">
                      {user.roles.length > 0
                        ? user.roles
                            .map(({ role }) => (role.isActive ? role.name : role.name + " (inactivo)"))
                            .join(", ")
                        : "Sin roles"}
                    </td>
                    <td className="px-6 py-3.5 text-slate-600 dark:text-slate-300">
                      {user.lastLoginAt ? dateFormatter.format(user.lastLoginAt) : "Nunca"}
                    </td>
                    <td className="px-6 py-3.5">
