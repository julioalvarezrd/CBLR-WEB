    ...
    <main className="flex min-h-screen flex-col bg-slate-100 px-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="flex flex-1 items-center justify-center py-12">
        <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <SiborBrand />
          <h1 className="mt-8 text-3xl font-bold text-slate-900 dark:text-white">Acceso institucional</h1>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Utiliza las credenciales administradas por {appConfig.name}.</p>

          {params.setup === "1" ? <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/50 dark:text-emerald-200">Configuración inicial completada. Ya puedes iniciar sesión.</div> : null}
          ...
          {initialSetupAvailable ? <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">¿Es la primera instalación?{" "}<Link href="/setup" className="font-medium text-slate-800 underline dark:text-slate-200">Configurar administrador inicial</Link></p> : null}
        </section>
      </div>
      <AppFooter />
    </main>
