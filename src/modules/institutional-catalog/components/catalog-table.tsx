...
export function CatalogTable<T extends { id: string; isActive: boolean }>({
  items,
  columns,
  emptyMessage,
  entity,
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-left text-sm">
        <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-950/50 dark:text-slate-400">
          <tr>
            {columns.map((column) => (
              <th key={column.label} className="whitespace-nowrap px-5 py-3 font-semibold sm:px-6">
                {column.label}
              </th>
            ))}
            <th className="px-5 py-3 font-semibold sm:px-6">Estado</th>
            {canManage ? <th className="px-5 py-3 text-right font-semibold sm:px-6">Acción</th> : null}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
          {items.map((item) => (
            <tr key={item.id} className="transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
              {columns.map((column) => (
                <td key={column.label} className="whitespace-nowrap px-5 py-4 text-slate-700 dark:text-slate-200 sm:px-6">
                  {column.render(item)}
                </td>
              ))}
