...
export async function listUsers(filters: { query?: string; status?: UserStatusFilter } = {}) {
  ...
  return prisma.user.findMany({
    ...
    orderBy: { name: "asc" },
    select: { id: true, name: true, email: true, isActive: true, lastLoginAt: true, roles: { select: { role: { select: { id: true, name: true, isActive: true } } } } },
  });
}

export async function getUser(userId: string) {
  await requirePermission("usuarios.view");
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, name: true, email: true, isActive: true, lastLoginAt: true, roles: { select: { role: { select: { id: true, name: true, isActive: true } } } } } });
...
export async function createUser(input: { name: string; email: string; password: string; roleIds: string[] }) {
  const actor = await requirePermission("usuarios.manage");
  ...
  return prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { name, email, passwordHash }, select: { id: true, name: true, email: true } });
    if (roleIds.length > 0) await tx.userRole.createMany({ data: roleIds.map((roleId) => ({ userId: user.id, roleId, assignedById: actor.user.id })) });
    await writeAudit(tx, { actorUserId: actor.user.id, action: "user.created", entityType: "User", entityId: user.id, after: { name: user.name, email: user.email, roleIds } });
    return user;
  });
}
